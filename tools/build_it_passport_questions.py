#!/usr/bin/env python3
"""ITパスポート Excel から assets/data/it-passport/*.json を生成する。"""
...
DATA_DIR = ROOT / "data" / "it-passport"
OUT_DIR = ROOT / "assets" / "data" / "it-passport"

DOWNLOADS = Path.home() / "Downloads"
XLSX = {
    "past": DOWNLOADS / "【最終盤】ITパスポート_過去問.xlsx",
    "drill": DOWNLOADS / "【最終盤】ITパスポート_一問一答.xlsx",
    "practice": DOWNLOADS / "【最終盤】ITパスポート_実践演習.xlsx",
    "mock": DOWNLOADS / "【最終盤】ITパスポート_有料模試.xlsx",
}
        ...
        questions.append(
            {
                ...
                "domain": pick(row, col, "分野", "章"),
                "topic": pick(row, col, "中項目", "トピック"),
        ...
        questions.append(
            {
                ...
                "choices": {
                    "A": pick(row, col, "選択肢1", "選択肢A", "選択肢ア"),
                    "B": pick(row, col, "選択肢2", "選択肢B", "選択肢イ"),
                    "C": pick(row, col, "選択肢3", "選択肢C", "選択肢ウ"),
                    "D": pick(row, col, "選択肢4", "選択肢D", "選択肢エ"),
                },
                "answer": normalize_choice_answer(pick(row, col, "正解", "正答")),
                "explanation": pick(row, col, "解説"),
            }
        )
...
def build_choice_row(header: list[str], row: tuple, no: int) -> dict:
    col = index_map(header)
    ...
    return {
        ...
        "question": pick(row, col, "問題文"),
        "choices": {
            "A": pick(row, col, "選択肢A", "選択肢1", "選択肢ア"),
            "B": pick(row, col, "選択肢B", "選択肢2", "選択肢イ"),
            "C": pick(row, col, "選択肢C", "選択肢3", "選択肢ウ"),
            "D": pick(row, col, "選択肢D", "選択肢4", "選択肢エ"),
        },
...
def build_past(path: Path) -> dict:
    ...
    for sheet_name in list_sheets(path):
        if sheet_name in {"問題バンク", "README", "説明"}:
            continue
        header, rows = load_sheet(path, sheet_name)
        if not rows:
            continue
        questions = [build_choice_row(header, row, i) for i, row in enumerate(rows, start=1)]
        exam_id = re.sub(r"[^a-z0-9]+", "-", sheet_name.lower()).strip("-") or "past"
...
def main() -> None:
    ...
    parser.add_argument("--past", type=Path, default=XLSX["past"])
