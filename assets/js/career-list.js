...
const INDEX_URL = '../data/career-index.json';

const CATEGORY_ORDER = ['role', 'learn', 'move', 'market'];
...
function parseState() {
  ...
  return {
    cat: cat === 'all' || CATEGORY_ORDER.includes(cat) ? cat : 'all',
...
function formatSalary(article) {
  if (article.salaryMin == null || article.salaryMax == null) return '';
  return `${article.salaryMin}〜${article.salaryMax}万円`;
}
...
function renderCategoryFilters(cat, q) {
  const row = document.getElementById('careerCategoryFilters');
  if (!row) return;
  const counts = countByCategory();
  const chips = [{ id: 'all', label: 'すべて' }, ...CATEGORY_ORDER.map((id) => ({ id, label: categories[id] || id }))];
  row.innerHTML = chips
    .map(({ id, label }) => {
...
function renderFeatured(cat, page, q) {
  const section = document.getElementById('careerFeatured');
  const grid = document.getElementById('careerFeaturedGrid');
...
function renderPagination(cat, page, q, totalPages) {
  const nav = document.getElementById('careerPagination');
  if (!nav) return;
  if (totalPages <= 1) {
    nav.innerHTML = '';
    return;
...
function render() {
  ...
  const range = document.getElementById('careerResultRange');
...
async function init() {
  try {
    ...
    allArticles = data.articles || data.roles || [];
  } catch (err) {
    const list = document.getElementById('roleList');
    if (list) {
